...
// Revised Brute-force Approach function
...
double MaxSubArraySum(STKprice A[], int n, int *low, int *high);

double RMaxSubArrayBF(STKprice A[], int n, int *low, int *high);
...
int main(void)
{
    ...
    t0 = getTime();				// Record the time of the beginning
    for (i = 0; i < Re; i++) {		// Repeat function 1000 times
        max = RMaxSubArrayBF(s, total, &low, &high);
    }
    low--;
    t1 = getTime();				// Record the end time.
    // Print the outcome
    printf("Revised Brute-force approach: CPU time %g s\n", (t1 - t0) / Re);
    printf("  Sell: %d/%d/%d",s[low].year, s[low].month, s[low].day);
    printf(" at %g\n", s[low].price);
    printf("  Buy: %d/%d/%d",s[high].year, s[high].month, s[high].day);
    printf(" at %g\n", s[high].price);
    ...
    t0 = getTime();				// Record the time of the beginning
    for (i = 0; i < Re; i++) {		// Repeat function 1000 times
        max = MaxSubArraySum(s, total, &low, &high);
    }
    low--;
    t1 = getTime();				// Record the end time.
    // Print the outcome
    printf("Kadane's algorithm: CPU time %g s\n", (t1 - t0) / Re);
    printf("  Sell: %d/%d/%d",s[low].year, s[low].month, s[low].day);
    printf(" at %g\n", s[low].price);
    printf("  Buy: %d/%d/%d",s[high].year, s[high].month, s[high].day);
    printf(" at %g\n", s[high].price);
...
double RMaxSubArrayBF(STKprice A[], int n, int *low, int *high)
{
    ...
    *low = 0;
    *high = n - 1;
    for (i = 0; i < n; i++) {		// Try all possible ranges: A[i : k]
        sum = 0;
        for (j = i; j < n; j++) {   // Summation for A[j : k]
            sum = sum + A[j].change;
            if (sum > max) {		// Record the minimum value and range
                max = sum;
                *low = i;
